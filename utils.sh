...
download_github_repo() {
  ...
  if [[ -z "$repo" || -z "$target_dir" ]]; then
    info "Usage: download_github_repo <user>/<repo> <target_dir>" >&2
    return 1
  fi
  ...
  info "[*] Directories temp: $tmp_dir, target: $target_dir"
...
#  trap rm -rf "$tmp_dir" EXIT
  ...
  if ! curl -L --fail --silent --show-error "$url_main" -o "$zip_path"; then
    info "[!] main.zip not found, trying master..."
    if ! curl -L --fail --silent --show-error "$url_master" -o "$zip_path"; then
      err "❌ Could not download archive from GitHub (tried main and master)." >&2
      return 1
    fi
  fi
